...
void test_dereference(PtrType ptr) {
    ...
    bit <<= 1;
    using PointeeType = typename std::remove_pointer<PtrType>::type;
    using FuncType = typename std::conditional<std::is_const<PointeeType>::value,
        void (PointeeType::*)(uint32_t) const,
        void (PointeeType::*)(uint32_t)>::type;
...
Status test_pointer() {
    ...
    {
        auto owner_a1 = unique_ptr<A>(new A);
        auto owner_a2 = unique_ptr<A>(new A);

        constexpr bool prototype_is_const = is_const<PrototypePointee>::value;

        using PointeeType = typename conditional<prototype_is_const, A const, A>::type;
...
Status test_copy_and_move() {
    ...
    // NonOwningPtr<Unrelated> other(nopa); // doesn't compile
...
Status test_mixed_comparison(PtrType1 ptr1, PtrType2 ptr2, expect_not_comparable) {
    ...
    //
    // (bool)(ptr1 == ptr2); // doesn't compile
    // (bool)(ptr1 != ptr2); // doesn't compile
    // (bool)(ptr1 < ptr2); // doesn't compile
    // (bool)(ptr1 > ptr2); // doesn't compile
    // (bool)(ptr1 <= ptr2); // doesn't compile
...
Status test_mixed_types() {
    ...
    {
        auto a_owner = unique_ptr<A>(new A);
        auto b_owner = unique_ptr<B>(new B);
        ...
        auto bb_owner = unique_ptr<BB>(new BB);
        BB * bb = bb_owner.get();
        NonOwningPtr<BB> nbb = bb;
        (void) nbb;

        assert(bb != b);
        // bb == a; // doesn't compile

        assert(nbb != nb);
        // (bool)(nbb == na); // doesn't compile

        assert(nbb != b);
        assert(b != nbb);
        // (bool)(nbb == a); // doesn't compile
        // (bool)(a == nbb); // doesn't compile

        // NonOwningPtr<B> otherb1(a); // doesn't compile
        // NonOwningPtr<B> otherb2(na); // doesn't compile
        ...
        // NonOwningPtr<BB> otherbb1(ncbb); // doesn't compile
        NonOwningPtr<BB> nbb2; (void)nbb2;
        // nbb2 = ncbb; // doesn't compile
        // NonOwningPtr<B> otherb3(ncbb); // doesn't compile
        NonOwningPtr<B> nb6; (void)nb6;
        // nb6 = ncbb; // doesn't compile

        struct BBx : B { };
        NonOwningPtr<BBx> nbbx; (void)nbbx;
        // (bool)(nbbx == nbb); // doesn't compile


        void * vptr = nb; (void)vptr;
        ...
        // delete invis; // doesn't compile
        // delete cinvis; // doesn't compile

        auto func = [](Invisible *) { };
        auto cfunc = [](Invisible const *) { };
        func(invis);

        // func(cinvis); // doesn't compile

        cfunc(cinvis);
        cfunc(invis);

        vptr = invis;
        // vptr = cinvis; // doesn't compile
